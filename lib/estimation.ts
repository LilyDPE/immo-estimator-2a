...
export async function estimateProperty(
  ...
  let geocoding;
  try {
    geocoding = await geocodeAddress(
  ...
  if (comparables.length < 3) {
    throw new Error(
      'Pas assez de données comparables trouvées dans ce secteur. Essayez d\'élargir la zone.'
    );
  }

  const weightedPricesPerSqm = comparables.map((sale) => {
    const distanceWeight = Math.max(0.1, 1 - (sale.distance! / 2000));
  ...
  const sortedPrices = weightedPricesPerSqm
  ...
  const adjustedMedianPricePerSqm = medianPricePerSqm + 
    adjustments.dpe + 
    adjustments.floor / property.surface;
  
  const basePrice = adjustedMedianPricePerSqm * property.surface;
  
  const estimatedPrice = {
    low: Math.round((lowPricePerSqm * property.surface + adjustments.floor + adjustments.cellar) / 1000) * 1000,
    median: Math.round((basePrice + adjustments.parking + adjustments.cellar + adjustments.balcony) / 1000) * 1000,
    high: Math.round((highPricePerSqm * property.surface + adjustments.parking + adjustments.cellar + adjustments.balcony + adjustments.floor) / 1000) * 1000,
  };
  ...
  const confidenceScore = calculateConfidenceScore(comparables, geocoding.latitude, geocoding.longitude);
...
function calculateAdjustments(
  ...
  if (property.dpe) {
    const dpeAdjustments: Record<DPEGrade, number> = {
      'A': 0.10,
      'B': 0.05,
      'C': 0,
      'D': 0,
    ...
    dpeAdjustment = basePricePerSqm * dpeAdjustments[property.dpe];
  }

  if (property.floor !== undefined) {
    if (property.floor === 0) {
      floorAdjustment = -5000;
    } else if (property.floor > 4) {
      const floorsAbove4 = property.floor - 4;
      floorAdjustment = Math.min(floorsAbove4 * 3000, 15000);
    }
  }
  ...
  return {
    ...
    parking: property.hasParking ? 15000 : 0,
    cellar: property.hasCellar ? 5000 : 0,
    balcony: property.hasBalcony ? 10000 : 0,
  };
}
...
function calculateConfidenceScore(
  ...
  const distanceScore = Math.max(0, 30 - (avgDistance / 500) * 30);
  score += distanceScore;
